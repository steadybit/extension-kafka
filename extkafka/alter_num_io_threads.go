...
func (k *AlterNumberIOThreadsAttack) Start(ctx context.Context, state *AlterNumberIOThreadsState) (*action_kit_api.StartResult, error) {
	...
	_, err = configs.On(strconv.FormatInt(int64(state.BrokerID), 10), func(resourceConfig *kadm.ResourceConfig) error {
		...
		for i := range resourceConfig.Configs {
			if resourceConfig.Configs[i].Key == NumberIOThreads {
	...
	if state.InitialBrokerConfigValue == "" {
		log.Warn().Msgf("No initial value found for configuration key: "+NumberIOThreads+", for broker node-id: %d", state.BrokerID)
	}

	// If initial value is retrieved without errors, proceed with alter config
	responses, err := adminClient.AlterBrokerConfigs(ctx, []kadm.AlterConfig{{Name: NumberIOThreads, Value: extutil.Ptr(state.BrokerConfigValue)}}, state.BrokerID)
	if err != nil {
		return nil, err
	}
	var errs []error
